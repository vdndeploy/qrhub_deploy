...
const Section = ({ title, icon: Icon, children, accent = 'orange' }) => {
  ...
  return (
    <section className="bg-white dark:bg-[#131316] border border-gray-200 dark:border-white/10 rounded-xl p-5 sm:p-6 mb-4">
      <h2 className={`text-lg font-semibold flex items-center gap-2 mb-2 ${colors[accent]?.split(' ')[2] || ''}`}>
        {Icon && <Icon className="h-5 w-5" />}{title}
      </h2>
      <div className="text-sm text-gray-700 dark:text-[#a8a8b0] space-y-3">{children}</div>
    </section>
  );
};
  ...
  return (
    <div className="max-w-3xl" data-testid="my-account-page">
      <header className="mb-4">
        ...
        <p className="text-sm text-gray-500 dark:text-[#6a6a72] mt-1">Esercita i tuoi diritti GDPR e gestisci la sicurezza del tuo account.</p>
      </header>
      ...
      <Section title="Cambia password" icon={Lock} accent="orange">
        ...
        <form onSubmit={handlePasswordChange} className="space-y-3 max-w-md" data-testid="change-password-form">
          ...
          <div>
            <Label className="text-xs">Nuova password <span className="text-gray-500 dark:text-[#6a6a72]">(min 8 caratteri)</span></Label>
            <Input
              type="password"
      ...
      {dpa && dpa.required && (
        <Section title="Data Processing Agreement (DPA)" icon={KeyRound} accent="emerald">
          ...
          {dpa.accepted_at && (
            <p className="text-xs text-gray-500 dark:text-[#6a6a72]">Accettato il: {dpa.accepted_at}</p>
          )}
