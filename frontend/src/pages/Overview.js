...
const Overview = () => {
  ...
  return (
    <div className="space-y-8" data-testid="overview-page">
      <div>
        <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-gray-900">Panoramica Globale</h2>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
          <StatCard icon={<Users className="h-6 w-6 text-[#F96815]" />} bg="bg-orange-100" label="Venditori" value={stats?.total_vendors} testid="stat-vendors" />
        ...
        {chartData.length > 0 && (
          <div className="bg-white rounded-lg border border-gray-200 p-6 mt-6" data-testid="analytics-chart">
            <h3 className="text-xl font-semibold mb-6 text-gray-900">Performance per Venditore</h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={chartData}>
                ...
                <YAxis />
                <Tooltip />
                <Bar dataKey="Visite" fill="#F96815" />
...
const StatCard = ({ icon, bg, label, value, testid }) => (
  <div className="bg-white rounded-lg border border-gray-200 p-6" data-testid={testid}>
    <div className="flex items-center gap-4">
      ...
      <div>
        <p className="text-sm text-gray-600 uppercase tracking-widest">{label}</p>
