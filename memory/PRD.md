...
- **Dark + Light theme con toggle**: shadcn `.dark` mode + variabili CSS riconfigurate con palette lime. Tutti i file del dashboard (Login, Dashboard, Overview, Organizations, Vendors, Stores, Files, Settings, OrgSettings, MyAccount, Legal, Dpa, AnalyticsDetailed, VendorLogin, VendorDashboard, PostsManager, MediaPicker) supportano dual-mode tramite `dark:` modifier Tailwind: `bg-white dark:bg-[#131316]`, `text-gray-900 dark:text-white`, ecc. Hex arancione `#F96815` → lime `#D2FA46` (presente in entrambi i modi). Recharts colors aggiornati (lime + purple). Hook `useTheme` (`/hooks/useTheme.js`) gestisce persistenza in localStorage + applica la classe `dark` su `<html>` al boot. Toggle button (Sun/Moon icon, `data-testid="theme-toggle-button"`) nell'header del Dashboard. Default = dark. Le landing pubbliche `/v/*` mantengono la propria palette per-org.
