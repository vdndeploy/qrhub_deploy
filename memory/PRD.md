...
| 2026-05-29 | **Deploy Fly v48 → fix PDF Log Eventi in Europe/Rome.** Verificato in produzione: PDF mostra `29/05/2026 13:30` invece di `2026-05-29 11:30` UTC; footer "Report generato il ... (ora Italia) — QRHub". | ✅ Fly v48 |
| 2026-05-29 | **Badge "NEW" su voce menu "Annunci"** (`Dashboard.js`): pill lime arrotondata, si inverte quando il tab è attivo. `data-testid="nav-posts-new-badge"`. | ✅ |
| 2026-05-29 | **Daily Counter chart scrollabile orizzontalmente** (`DailyCounterCard.js`): wrapper `vendors-chart-scroll` con slot fisso 48px/giorno (44px/ora) → con 30/90gg il grafico mostra tutte le label senza accavallamenti. `interval=0` su XAxis. | ✅ |
| 2026-05-29 | **GDPR hardening DPA v1.0 → v1.1** (`Dpa.js`, `server.py::CURRENT_DPA_VERSION`): aggiunte clausole §1 "Natura non commerciale", §6 "Responsabilità del Cliente" (Controller liable verso interessati/Garante), §9 "Limitazione responsabilità ed esclusione di garanzia" potenziata (AS-IS, AS-AVAILABLE, esonero da danni indiretti/sanzioni Garante, valore prestazione = 0), §10 "Backup autonomo", §12 "Modifiche DPA", §13 "Foro Verona". Tutti gli org admin che avevano accettato v1.0 vengono ri-promtati al prossimo login. | ✅ |
| 2026-05-29 | **Backup tab nel Super Admin** (`/api/super-admin/backup/db` + `/backup/github`, nuovo router `routers/super_admin.py`, UI `Settings.js` tab "Backup"): export ZIP completo MongoDB (Extended JSON con manifest + README mongoimport) + snapshot zipball GitHub via API ufficiale. La vecchia tab "GitHub" è stata rinominata in "Backup" e ora ospita anche i pulsanti download. | ✅ |
| 2026-05-29 | **Free-tier Usage Monitor** (`/api/super-admin/usage` + tab "Usage" in Settings): chiama in parallelo Fly GraphQL, Cloudinary /usage, Vercel /deployments, MongoDB Atlas Admin API (Digest auth, opzionale). Rende 4 card con progress bar % e tono colore (verde <70%, arancio 70-90%, rosso ≥90%). Limiti free tier hardcoded in `FREE_LIMITS`. Campi `atlas_public_key/private_key/group_id` aggiunti al `DeployConfig`. | ✅ |
| 2026-05-29 | **Fix MongoDB card "rossa"**: la barra "1/1 cluster" diventava rossa perché interpretata come consumo 100%. Sostituita con barra "Storage DB" reale (via `dbStats` su `motor`, niente API extra) — `0.93 / 512 MB (0.2%)` verde. Aggiunto status `partial` per quando c'è solo connessione DB e non chiavi Atlas. Tutte le card mostrano `%` inline. | ✅ |
| 2026-05-29 | **Billing & cost summary nel tab Usage**: banner verde in cima con "Costo questo mese (provider noti) $X/mese". Card Fly mostra plan badge ("Hobby pay-as-you-go") + nota onesta ("costo visibile solo nel dashboard Fly perché GraphQL non lo espone"). Card Atlas/Cloudinary/Vercel mostrano `$0.00/mese` quando piano Free. Link "Apri billing dashboard" per ogni provider. Verificato live: Atlas $0, Cloudinary $0, Vercel $0, Fly = uso reale visibile sul dashboard (utente lasciato consapevole). | ✅ Fly v51 |
